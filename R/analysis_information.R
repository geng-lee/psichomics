...
renderGeneticInfo <- function(output, info, species=NULL, assembly=NULL,
    ...
    links <- prepareExternalLinks(info, species, assembly, grch37, gene, event)
...
prepareExternalLinks <- function(info, species, assembly, grch37, gene, event) {
    linkTo <- function(title, href) {
        tags$a(title, icon("external-link"), target="_blank", href=href)
    }

    eventData <- findEventData(event)
    isVASTTOOLS <- FALSE
    # if (!is.null(eventData)) {
    #     isVASTTOOLS <- isTRUE(any(eventData$source == "vast-tools"))
    # }
    vastdb <- NULL
    if (isVASTTOOLS) {
        vastdb <- sprintf("http://vastdb.crg.eu/wiki/Event:%s@Genome:%s",
                          event, assembly)
    }
    ...
    if (!is.null(info)) {
        ...
        if (is.null(vastdb)) {
            vastdb <- sprintf("http://vastdb.crg.eu/wiki/Gene:%s@Genome:%s",
                              gene, assembly)
        }
        url$vastdb <- vastdb
    } else {
        ...
        if (is.null(vastdb)) {
            vastdb <- paste0("http://vastdb.crg.eu/wiki/search?query=", gene)
        }
        url$vastdb <- vastdb
    }
    ...
    observe({
        ...
        output$genetic <- renderUI({
            ...
            geneticInfo <- renderGeneticInfo(
                output, info, species, assembly, grch37, event=event,
